  ...
  ({ className, label, helperText, error, disabled, id, ...props }, ref) => {
    ...
    return (
      <div className={cn('flex flex-col gap-1.5', disabled && 'opacity-50')}>
        {label && (
          <label htmlFor={inputId} className="text-sm font-medium leading-[14px] text-slate-900">
            {label}
          </label>
        )}
        <textarea
          ...
          disabled={disabled}
          className={cn(
            'min-h-[80px] w-full resize-none rounded-[6px] border border-slate-300 bg-white px-3 py-2',
